...
$(document).ready(function () {
  $("body").on("keydown", ".roundto2", function (evt) {
    ...
    if (!(removedcodes.indexOf(charCode) > -1)) {
        ...
        if (obj.status == "error") {
          ...
          $.each(itemlist, function (index, item) {
            if (mSizeId && item.id_iq_material_size == mSizeId) {
              $("#material_size_id").append(
                $("<option></option>")
                  .attr("value", item.id_iq_material_size)
  ...
  function getProductSuggetion() {
    ...
    var material = $('input[name="material_type"]:checked').attr("data-attr-name");
    ...
    suggesteProdAjax = $.ajax({
      ...
      type: "post",
      data: { ajax: true, length: l, height: h, width: w, material:material },
      success: function (res) {
        obj = $.parseJSON(res);
  ...
  function validate() {
    ...
    $(
      "#material_type_form input:text,#material_swiper select, #material_swiper input:radio"
    ).each(function (index) {
      if (!$(this).val()) {
        formValidated = false;
  ...
  $("body").on("click", "#addtocart", function () {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (obj.status == "error") {
          $("#extraerrors").append(
            '<div class="error">Some technical issue occurred, Please try after sometime ...</div>'
          );
          $("#extraerrors").show();
        } else {
...
function initializeSwiperSuggestedProd() {
  relProSwiper = new Swiper(
    ".suggested_products_container .pro_gallery_suggested_prod",
    {
      ...
      onSlideChangeEnd: function (swiper) {
        if (typeof $(".pro_gallery_suggested_prod ")[0].swiper !== "undefined")
          $(".pro_gallery_suggested_prod ")[0].swiper.slideTo(
            swiper.activeIndex
          );
      },
